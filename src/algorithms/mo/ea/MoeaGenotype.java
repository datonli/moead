...
import algorithms.mo.IMultiObjectiveSolver;
import algorithms.random.RanMT;


public abstract class MoeaGenotype implements IMultiObjectiveSolver {
	...
	protected IMultiObjectiveProblem mop;
	...
	public IMultiObjectiveProblem getMultiObjectiveProblem() {
		if (mop == null)
	...
	public void setMultiObjectiveProblem(IMultiObjectiveProblem problem) {
		this.mop = problem;
	}
	...
	public void evaluate(MoChromosome chromosome) {
		IMultiObjectiveProblem multiObjectiveProblem = this
				.getMultiObjectiveProblem();
		multiObjectiveProblem.evaluate(chromosome);
		this.EvalCounter++;
	};
	...
	public final void solve(IMultiObjectiveProblem problem) {
		this.setMultiObjectiveProblem(problem);
		this.setNumObjectives(problem.getObjectiveSpaceDimension());
		this.setNumParameters(problem.getParameterSpaceDimension());
		this.doSolve();
		this.stoped = true;
	}
...
//	计算欧几里德距离，目的是：根据权向量的欧氏距离大小来确定权向量远近，最终找出相邻权向量集。
